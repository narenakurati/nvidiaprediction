...
Moore’s Law predicts that the number of transistors per square inch doubles each 18 months. This may be an interesting metric in predicting future Nvidia transistor density. Let’s create another vector for what Moore’s Law predicts the transistor density to be taking the GTX 280’s transistor density as our starting point.
```{r}
...
ggplot(transistor_df, aes(x = date, y = transistors, color = type)) + geom_point() + geom_smooth(se=FALSE) + scale_color_manual(values=c("#76b900", "royalblue1")) + xlim(as.Date("2009-01-08"), as.Date("2018-08-02")) + ylim(0, 500000) + labs(x = "time", y = "transistor_density")
```

As far as predicting the GTX 1180's transistor density, we can very clearly see that Moore's Law is not going to help us here. Let's create a linear model regressing past GTX graphics cards' transistor densities against time. While the specific release date of each graphics card is definitely important, it's better to avoid using them as factors. Instead we take the response variable as continuous. THis helps us a lot more with our model's accuracy, especially since, to us, the release date of each graphics card is more or less arbitrary and in the control of Nvidia. 
```{r}
...
A linear fit seems to be pretty good, and we have a significant slope with an incredibly low p-value. However, this can be made better. Now while we saw Nvidia's transistor density improvments don't happen at the level Moore's Law suggests, the growth is certainly more rapid than a linear model would accurately predict. Power transform close to 0.5 suggests a square root transformation on y. Our new model is now more accurate.
```{r}
library(car)
...
Now let's use both models to predict transistor densities for a an x-value of August 1, 2018 (assuming the new GTX 1180 will come out in the next few weeks).
```{r}
#predicted performance boost by transistor density through nvidia linear model
predict(nvidia_transistor_model, newdata = data.frame("date" = as.Date("2018-08-01")), type = "response")/nvidia_transistors[7,2]

#predicted performance boost by transistor density through nvidia transformed model
predict(nvidia_transistor_model_2, newdata = data.frame("date" = as.Date("2018-08-01")), type = "response")^2/nvidia_transistors[7,2]
...
```{r}
ggplot(nvidia_transistors, aes(x = date, y = transistors,)) + geom_point() + geom_smooth(se=FALSE,  color = "#76b900") + xlim(as.Date("2009-01-08"), as.Date("2018-8-02")) + ylim(0, 50000) + labs(x = "time", y = "transistor_density") + geom_point(aes(x=as.Date("2018-08-01"), y=predict(nvidia_transistor_model, newdata = data.frame("date" = as.Date("2018-08-01")), type = "response")), shape = 15, color = "black", size = 2) + geom_point(aes(x=as.Date("2018-08-01"), y=predict(nvidia_transistor_model_2, newdata = data.frame("date" = as.Date("2018-08-01")), type = "response")^2), shape = 17, color = "black", size = 2) 


#switched models, careful below

# + geom_label(aes(x=as.Date("2017-09-01"), y=predict(nvidia_transistor_model, newdata = data.frame("date" = as.Date("2018-08-01")), type = "response")^2), label = "transformed") + geom_label(aes(x=as.Date("2018-01-01"), y=predict(nvidia_transistor_model2, newdata = data.frame("date" = as.Date("2018-08-01")), type = "response")), label = "linear")
