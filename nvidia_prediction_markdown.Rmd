...
game_synthetic_scores <- data.frame("date" = as.Date(xx80$launch), "battlefield1" = xx80$battlefield1, "tflop" = xx80$processing_power_single, "threedmark" = xx80$threedmark_score)

game_synthetic_model <- lm(battlefield1 ~ sqrt(tflop) + threedmark, data = game_synthetic_scores)
summary(game_synthetic_model)
```
